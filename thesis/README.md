...
1. **Decoupled Asynchronous Inference Pipeline:** An OpenCV capture-draining thread (`CAP_PROP_BUFFERSIZE = 1`) coupled to a background neural inference engine, achieving zero frame latency and constant 30 FPS playback.
2. **Dynamic Multi-Tracker Suite with Scene Analysis:** An adaptive tracking engine combining **ByteTrack**, **BoT-SORT**, **OC-SORT**, and **FastTracker**, governed by an optical flow and pairwise Intersection-over-Union (IoU) scene analyzer with a 3.0-second hysteresis buffer.
3. **Illumination-Invariant Contrast Normalization:** A LAB color-space Contrast Limited Adaptive Histogram Equalization (**CLAHE**) pipeline enhancing low-light triage visibility without chromatic distortion.
4. **Centroid Spatial Fallback & Temporal Debouncing:** A spatial Euclidean re-identification mechanism (`radius = 40.0 px`) and a 5.0-second lost-track debouncing queue that prevents target identity flicker and double-counting during severe occlusion.
5. **High-Efficiency ONNX Runtime Deployment:** Quantized and ONNX-optimized neural architectures achieving a **2.1x CPU inference speedup** (28.4 ms vs. 59.8 ms on baseline PyTorch), allowing deployment on standard hospital workstations without discrete GPUs.
6. **Reactive Monolithic UI & Automated Auditing:** A zero-network-serialization Python monolith built on NiceGUI, featuring real-time telemetry binding, dynamic camera and AI model switching, capacity threshold alerting, and automated PDF/CSV clinical audit generation.
...
| **Chapter 2** | [`02_LITERATURE_REVIEW.md`](02_LITERATURE_REVIEW.md) | Evolution of Object Detection, Multi-Object Tracking State-of-the-Art, CLAHE Illumination, Perspective Geometry |
| **Chapter 3** | [`03_SYSTEM_ARCHITECTURE.md`](03_SYSTEM_ARCHITECTURE.md) | Monolithic Architecture, Decoupled Dual-Thread Pattern, In-Memory State Binding, Hardware Abstraction |
| **Chapter 4** | [`04_COMPUTER_VISION_AND_DEEP_LEARNING.md`](04_COMPUTER_VISION_AND_DEEP_LEARNING.md) | Dataset Engineering, Fine-Tuning YOLO, ONNX Runtime Optimization, Dynamic Class Mapping, LAB CLAHE |
| **Chapter 5** | [`05_TRACKING_SCENE_ANALYSIS_AND_DEBOUNCING.md`](05_TRACKING_SCENE_ANALYSIS_AND_DEBOUNCING.md) | Mathematical Formulations of Trackers, Scene Analyzer (Motion & Occlusion), Hysteresis, Spatial Fallback |
| **Chapter 6** | [`06_CLINICAL_MONITORING_AND_USER_INTERFACE.md`](06_CLINICAL_MONITORING_AND_USER_INTERFACE.md) | Telemetry Computation, Overcrowding Alerts, NiceGUI Clinical Interface Tokens, Automated PDF/CSV Reports |
| **Chapter 7** | [`07_EXPERIMENTS_BENCHMARKS_AND_RESULTS.md`](07_EXPERIMENTS_BENCHMARKS_AND_RESULTS.md) | Experimental Methodology, Detection mAP, Tracking Metrics (MOTA/IDF1/HOTA), Latency & FPS, Ablation Studies |
...
| **Appendices** | [`11_APPENDICES.md`](11_APPENDICES.md) | Configuration Schema, Mathematical Derivations, Unit Test Suite & Codebase Structure |
...
```
                       ...
                       │   - YOLO Deep Feature Inference (ONNX)  │
